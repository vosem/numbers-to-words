...
const Keyboard = ({ setWords }: Props): JSX.Element => {
  ...
  const sendRequest = async (numbersParameter: string): Promise<void> => {
    try {
      const response = await fetch(`${URL}?${new URLSearchParams({ numbers: numbersParameter }).toString()}`);
      ...
      <div className={styles.Keyboard}>
        {buttons.map((button, index) => {
          ...
          return (
            <button
              key={number}
              onClick={index === buttons.length - 1 ? removeNumber : (): Promise<void> => convert(number)}
